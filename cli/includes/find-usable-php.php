...
// First, check if the system's linked "php" is the required version; if so, return that. This
// is the most likely, most ideal, and fastest possible case
$linkedPhpVersion = shell_exec('php -r "echo phpversion();"');

if (version_compare($linkedPhpVersion, $runPhpVersion) >= 0) {
    // Current linked php is the correct version.
    echo exec('which php');

    return;
}

// If not, let's find it whether we have the required version of PHP installed;
...
// If we don't have the required run version of PHP, throw an error
if (array_search($runPhpVersion, $phps) === false) {
    throw new Exception("Sorry, but you don't have the required version of PHP ($runPhpVersion) installed to run Valet Pro Max.");
}

echo getPhpExecutablePath(array_search($runPhpVersion, $phps));
return;


// Filter out older versions of PHP
$modernPhps = array_filter($phps, function ($php) use ($runPhpVersion) {
    return version_compare($php, $runPhpVersion) >= 0;
});
